...
describe('aigate agent detail handlers', () => {
  ...
  describe('agent [id].get', () => {
    it('should return 404 when agent not found', async () => {
      ...
      const response = await agentGetHandler(
        createMockEvent({
    ...
    it('should return agent scoped to organization', async () => {
      ...
      expect(response.data).toEqual(agent)
      expect(mockSelect).toHaveBeenCalledTimes(1)
    })
  ...
  describe('agent [id]/conversations.get', () => {
    it('should return unauthorized when user is not logged in', async () => {
      const response = await agentConversationsHandler(
        createMockEvent({
          params: { id: 'agent-1' },
        }),
      )
