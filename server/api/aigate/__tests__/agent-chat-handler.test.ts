...
describe('aigate agent chat handler', () => {
  beforeEach(() => {
    ...
    mockStreamAgentMessage.mockReturnValue(
  ...
  it('should return 401 when user is not authenticated', async () => {
    const response = asResponse<ErrorData>(
      await agentChatHandler(
        createMockEvent({
          params: { id: 'agent-1' },
          body: { message: 'hello' },
        }),
  ...
  it('should return success response for non-stream requests', async () => {
    const response = asResponse<ChatSuccessData>(
      await agentChatHandler(
        createMockEvent({
          context: { principal: { userId: 'user-1', organizationId: 'org-1' } },
          params: { id: 'agent-1' },
  ...
  it('should write SSE chunks and DONE marker during stream', async () => {
    const response = (await agentChatHandler(
      createMockEvent({
        context: { principal: { userId: 'user-1', organizationId: 'org-1' } },
        params: { id: 'agent-1' },
        body: { message: 'hello', stream: true },
