...
describe('aigate prompt extended handlers', () => {
  ...
  describe('prompt import.post', () => {
    it('should reject non-array import data', async () => {
      const response = await promptImportHandler(
        createMockEvent({
    ...
    it('should skip items missing name or content', async () => {
      ...
      const response = await promptImportHandler(
        createMockEvent({
          context: { principal: { organizationId: 'org-1' } },
          body: [{ name: 'Valid', content: 'OK' }, { name: 'No content' }, { content: 'No name' }],
        }),
      )
  ...
  describe('prompt export.get', () => {
    it('should export prompts scoped to organization', async () => {
      ...
      expect(mockSetResponseHeader).toHaveBeenCalledWith(
        event,
        'Content-Disposition',
        'attachment; filename="prompts-export.json"',
      )
    })
    ...
    it('should allow admin to export all prompts without organization context', async () => {
      ...
      const result = await promptExportHandler(
        createMockEvent({
  ...
  describe('prompt [id].put', () => {
    it('should return 404 when prompt not found', async () => {
      ...
      const response = await promptPutHandler(
        createMockEvent({
          context: { principal: { organizationId: 'org-1' } },
          params: { id: 'missing' },
          body: { name: 'Updated' },
  ...
  describe('prompt [id]/versions index.get', () => {
    it('should return 404 when prompt not found', async () => {
      ...
      const response = await promptVersionsHandler(
        createMockEvent({
          context: { principal: { organizationId: 'org-1' } },
          params: { id: 'missing' },
        }),
  ...
  describe('prompt [id]/versions/[versionId] restore.post', () => {
    it('should return 404 when prompt not found', async () => {
      ...
      const response = await promptRestoreHandler(
        createMockEvent({
          context: { principal: { organizationId: 'org-1' } },
