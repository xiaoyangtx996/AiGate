...
export default defineEventHandler(async event => {
  try {
    ...
    const keys = orgFilter ? await db.select().from(apiKey).where(orgFilter) : await db.select().from(apiKey)
    ...
    const recentRequests = logOrgFilter
      ? await db
          ...
          .from(apiLog)
          .where(and(logOrgFilter, gte(apiLog.createdAt, oneHourAgo)))
      : await db
          .select({ count: sql<number>`count(*)::int` })
          .from(apiLog)
          .where(gte(apiLog.createdAt, oneHourAgo))
