...
function scopeMenusByRolePermissions(
  ...
  for (const permission of permissions) {
    grantedPermissions.set(permission.menuId, (grantedPermissions.get(permission.menuId) ?? 0) | permission.permissions)
  }
...
export default defineEventHandler(async event => {
  try {
    ...
    // keyword 模糊搜索
    if (keyword) {
      conditions.push(or(ilike(menu.label, `%${keyword}%`), ilike(menu.to, `%${keyword}%`)))
    }

    // enabled 精确筛选
    if (enabled !== undefined) {
      conditions.push(eq(menu.enabled, enabled))
    }
    ...
    const data = await db
      ...
      .orderBy(asc(menu.createdAt), desc(menu.sort))
