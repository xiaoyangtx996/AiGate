...
describe('quota utils', () => {
  ...
  describe('checkQuota', () => {
    ...
    it('should allow unlimited usage when tokenLimit is zero', async () => {
      mockSelect.mockReturnValue(createSelectWhereChain([{ id: 'org-1', tokenLimit: 0, tokenUsed: 500 }]))
    ...
    it('should reject when quota is exhausted', async () => {
      mockSelect.mockReturnValue(createSelectWhereChain([{ id: 'org-1', tokenLimit: 1000, tokenUsed: 1000 }]))
    ...
    it('should reject when requested tokens exceed remaining', async () => {
      mockSelect.mockReturnValue(createSelectWhereChain([{ id: 'org-1', tokenLimit: 1000, tokenUsed: 950 }]))
    ...
    it('should allow when sufficient quota remains', async () => {
      mockSelect.mockReturnValue(createSelectWhereChain([{ id: 'org-1', tokenLimit: 1000, tokenUsed: 200 }]))
  ...
  describe('getQuotaStatus', () => {
    ...
    it('should return warning and critical flags based on usage percent', async () => {
      mockSelect.mockReturnValue(
        createSelectWhereChain([{ id: 'org-1', name: 'Alpha', tokenLimit: 1000, tokenUsed: 960 }]),
      )
    ...
    it('should return zero usage percent when tokenLimit is zero', async () => {
      mockSelect.mockReturnValue(createSelectWhereChain([{ id: 'org-2', name: 'Beta', tokenLimit: 0, tokenUsed: 500 }]))
  ...
  describe('validateQuotaConservation', () => {
    ...
    it('should reject when quota is lower than used tokens', () => {
      const result = validateQuotaConservation(orgs, [{ organizationId: 'company', nextTokenLimit: 40 }])
  ...
  describe('quota request workflow helpers', () => {
    ...
    it('should reject invalid quota request drafts', () => {
      expect(createQuotaRequest({ organizationId: '', requestedTokenLimit: 100, requesterId: 'user-1' })).toEqual({
        valid: false,
        reason: '缺少组织',
      })
      expect(createQuotaRequest({ organizationId: 'org-1', requestedTokenLimit: -1, requesterId: 'user-1' })).toEqual({
        valid: false,
        reason: '申请配额必须是非负整数',
      })
    })
  ...
  describe('quota request persistence workflow', () => {
    it('should create quota request with current organization token limit', async () => {
      ...
      mockSelect.mockReturnValue(createSelectWhereChain([{ id: 'org-1', tokenLimit: 1000, tokenUsed: 100 }]))
    ...
    it('should approve request, update quota and write change log', async () => {
      ...
      const orgs = [{ id: 'org-1', parentId: null, tokenLimit: 500, tokenUsed: 100 }]
      ...
      mockUpdate.mockReturnValueOnce(createUpdateChain()).mockReturnValueOnce(createUpdateReturningChain([updated]))
