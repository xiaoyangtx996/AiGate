...
function formatQuotaAlertMessage(
  orgName: string,
  usagePercent: number,
  tokenUsed: number,
  tokenLimit: number,
...
describe('alerts utils', () => {
  describe('quota alert logic', () => {
    ...
    it('should format quota alert resource, title and message', () => {
      ...
      expect(formatQuotaAlertMessage('Acme Corp', 92, 920, 1000, 90)).toBe(
        '组织 "Acme Corp" 配额使用已达 92%（920/1000 tokens），触发 90% 阈值',
      )
    })
  })
  ...
  describe('key expiry alert logic', () => {
    ...
    it('should format key expiry alert title and message', () => {
      ...
      expect(formatKeyExpiryAlertMessage('Production Key', expiresAt)).toBe(
        '密钥 "Production Key" 将于 2026-06-12 过期',
      )
    })
  })
  ...
  describe('rule threshold logic', () => {
    ...
    it('should format rule-based quota alert title and message', () => {
      ...
      expect(formatRuleQuotaAlertMessage('High Usage Rule', 'Acme Corp', 88)).toBe(
        '规则 "High Usage Rule" 触发：组织 "Acme Corp" 配额使用 88%',
      )
    })
    ...
    it('should format rule-based key expiry alert title and message', () => {
      ...
      expect(formatRuleKeyExpiryAlertMessage('Expiry Rule', 'Staging Key')).toBe(
        '规则 "Expiry Rule" 触发：密钥 "Staging Key" 即将过期',
      )
    })
