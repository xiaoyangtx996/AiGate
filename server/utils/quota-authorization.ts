...
export function canApproveQuotaRequest(principal: QuotaPrincipal | undefined, request: QuotaRequestScope) {
  if (!principal?.userId) return false

  if (principal.isAdmin) return true

  if (!QUOTA_APPROVER_ROLES.has(principal.role ?? '')) return false

  if (!principal.organizationId || principal.organizationId !== request.organizationId) return false
