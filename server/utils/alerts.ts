    ...
    if (tier > 0) {
      ...
      if (existing.length === 0) {
        const [newAlert] = await db
          ...
          .returning()
        if (newAlert) await notifyAlertSubscribers(newAlert.id, ['email']).catch(() => {})
      }
    }
  }
}
...
export async function generateKeyExpiryAlerts() {
  ...
  for (const key of expiringKeys) {
    const existing = await db
      .select()
      .from(alert)
      .where(and(eq(alert.type, 'key_expiring'), eq(alert.resourceId, key.id), eq(alert.read, false)))
    if (existing.length === 0) {
      const [newAlert] = await db
        .insert(alert)
        .values({
    ...
    if (rule.type === 'quota_warning') {
      ...
      for (const org of orgs) {
        ...
        const [newAlert] = await db
          ...
          .returning()
        if (newAlert) await notifyAlertSubscribers(newAlert.id, notifyChannels).catch(() => {})
      }
    }
