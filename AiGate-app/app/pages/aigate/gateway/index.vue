  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-2">
        <UButton icon="lucide:route" variant="outline" to="/aigate/gateway/routes">{{ p('routes') }}</UButton>
        <UButton icon="lucide:refresh-cw" variant="ghost" :loading="loading" @click="refresh()" />
      </div>
    </div>

    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <UCard><p class="text-sm text-muted">{{ p('activeKeys') }}</p><p class="text-2xl font-bold">{{ overview.activeKeys }}/{{ overview.totalKeys }}</p></UCard>
      <UCard><p class="text-sm text-muted">{{ p('healthyChannels') }}</p><p class="text-2xl font-bold text-success">{{ overview.healthyChannels }}/{{ overview.activeChannels }}</p></UCard>
      <UCard><p class="text-sm text-muted">{{ p('requestsLastHour') }}</p><p class="text-2xl font-bold">{{ overview.requestsLastHour || 0 }}</p></UCard>
      <UCard><p class="text-sm text-muted">{{ p('avgLatency') }}</p><p class="text-2xl font-bold">{{ overview.avgLatency || 0 }}ms</p></UCard>
    </div>

    <UCard>
      <template #header><h3 class="font-bold">{{ p('channelRoutes') }}</h3></template>
      ...
      />
      <UTable v-else :data="channels" :columns="[
        ...
        <template #health-cell="{ row }">
          <UBadge :color="healthColor[row.original.health] || 'neutral'" variant="subtle">{{ row.original.health }}</UBadge>
        </template>
    ...
    <UCard>
      <template #header><h3 class="font-bold">{{ p('recentLogs') }}</h3></template>
      <EmptyState
        ...
        :description="p('subtitle')"
      />
      <UTable v-else :data="recentLogs" :columns="[
