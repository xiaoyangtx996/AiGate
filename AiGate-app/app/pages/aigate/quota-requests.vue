...
const { data, pending: loading, refresh } = await useAsyncData('aigate-quota-requests', async () => {
  ...
  <div class="space-y-4">
    ...
    >
      <template #quota-cell="{ row }">
        <span class="font-mono">{{ formatTokens(row.original.currentTokenLimit) }} → {{ formatTokens(row.original.requestedTokenLimit) }}</span>
      </template>
      ...
      <template #status-cell="{ row }">
        <UBadge :color="row.original.status === 'approved' ? 'success' : row.original.status === 'rejected' ? 'error' : 'warning'" variant="subtle">
      ...
      <template #actions-cell="{ row }">
        <div v-if="row.original.status === 'pending' && row.original.canDecide !== false" class="flex gap-1">
          <UButton v-permission="'EDIT'" size="xs" color="success" variant="soft" @click="openDecision(row.original, 'approved')">
            {{ p('approve') }}
          </UButton>
          <UButton v-permission="'EDIT'" size="xs" color="error" variant="soft" @click="openDecision(row.original, 'rejected')">
            {{ p('reject') }}
          </UButton>
        </div>
      ...
      <template #body>
        <div class="space-y-4">
          <div v-if="selectedRequest" class="rounded border border-default p-3 text-sm">
            <p class="font-medium">
              {{ formatTokens(selectedRequest.currentTokenLimit) }} → {{ formatTokens(selectedRequest.requestedTokenLimit) }}
            </p>
            <p class="text-muted">
