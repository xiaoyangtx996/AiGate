...
function findMenuByPath(nodes: MenuPermissionNode[], path: string): MenuPermissionNode | null {
  ...
  for (const node of nodes) {
    ...
    const matchesNode = nodePath && nodePath !== '/' && !node.children?.length && normalizedPath.startsWith(`${nodePath}/`)
...
export function canUseMenuPermission(
  menuTree: MenuPermissionNode[],
  ...
  if (!target.permission && !target.anyOf?.length && !target.allOf?.length)
    return true

  const bits = menu.permissions ?? 0

  if (target.permission && !hasPermissionBit(bits, target.permission))
    return false

  if (target.anyOf?.length && !target.anyOf.some(permission => hasPermissionBit(bits, permission)))
    return false

  if (target.allOf?.length && !target.allOf.every(permission => hasPermissionBit(bits, permission)))
    return false
