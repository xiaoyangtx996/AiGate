...
export function useAigateApi() {
  ...
  // Dashboard
  const getDashboard = (params?: Record<string, unknown>) => get<DashboardResponse>('/aigate/dashboard', params, cacheConfig.stats)
  const getMyWorkbench = () => get<ApiRecord>('/aigate/me/workbench', undefined, cacheConfig.stats)
  const getMyApiLogList = (params?: Record<string, unknown>) => get<ListResponse>('/aigate/me/api-log', params, cacheConfig.list)
  const getMyApiKeyList = (params?: Record<string, unknown>) => get<ListResponse>('/aigate/me/api-key', params, cacheConfig.list)
  const createMyApiKey = (body: unknown) => post('/aigate/me/api-key', body)
  const updateMyApiKey = ({ id, ...body }: { id: string } & Record<string, unknown>) => put(`/aigate/me/api-key/${id}`, body)
  const getQuotaRequests = (params?: Record<string, unknown>) => get<ApiRecord[]>('/aigate/quota/request', params, cacheConfig.list)
  const createQuotaRequest = (body: unknown) => post('/aigate/quota/request', body)
  const decideQuotaRequest = ({ id, ...body }: { id: string } & Record<string, unknown>) => post(`/aigate/quota/request/${id}/decision`, body)

  // Organization
  const getOrgList = (params?: Record<string, unknown>) => get<ListResponse>('/aigate/organization', params, cacheConfig.list)
  ...
  // Member
  const getMemberList = (params?: Record<string, unknown>) => get<ListResponse>('/aigate/member', params, cacheConfig.list)
  const insertMember = (body: unknown) => post('/aigate/member', body)
  const delMember = (id: string) => del(`/aigate/member/${id}`)

  // Channel
  const getChannelList = (params?: Record<string, unknown>) => get<ListResponse>('/aigate/channel', params, cacheConfig.list)
  ...
  const checkChannelHealth = (channelId?: string) => post<HealthCheckResponse>('/aigate/channel/health-check', channelId ? { channelId } : {})

  // API Key
  const getApiKeyList = (params?: Record<string, unknown>) => get<ListResponse>('/aigate/api-key', params, cacheConfig.list)
  ...
  // Model
  const getModelList = (params?: Record<string, unknown>) => get<ListResponse>('/aigate/model', params, cacheConfig.list)

  // MCP Tool
  const getMcpToolList = (params?: Record<string, unknown>) => get<ListResponse>('/aigate/mcp-tool', params, cacheConfig.list)
  ...
  // Agent
  const getAgentList = (params?: Record<string, unknown>) => get<ListResponse<AgentListItem>>('/aigate/agent', params, cacheConfig.list)
  ...
  const chatWithAgent = (id: string, message: string, conversationId?: string) => post<ApiRecord>(`/aigate/agent/${id}/chat`, { message, conversationId })
  const getAgentConversations = (id: string) => get<ChatConversation[]>(`/aigate/agent/${id}/conversations`)

  // Knowledge Base
  const getKnowledgeBaseList = (params?: Record<string, unknown>) => get<ListResponse>('/aigate/knowledge-base', params, cacheConfig.list)
  ...
  // Prompt
  const getPromptList = (params?: Record<string, unknown>) => get<ListResponse>('/aigate/prompt', params, cacheConfig.list)
  ...
  // Alert
  const getAlertList = (params?: Record<string, unknown>) => get<ListResponse>('/aigate/alert', params, cacheConfig.list)
  ...
  // Billing
  const getBillingList = (params?: Record<string, unknown>) => get<ListResponse>('/aigate/billing', params, cacheConfig.list)
  ...
  // API Log
  const getApiLogList = (params?: Record<string, unknown>) => get<ListResponse>('/aigate/api-log', params, cacheConfig.list)
