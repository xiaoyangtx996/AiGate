...
export function responseError(data: unknown = null, msg?: string, options: ErrorResponseOptions = {}): IResponse {
  ...
  const message = isExplicitMessage
    ? msg
    : getErrorMessage(data, getStatusMessage(statusCode))

  applyResponseStatus(statusCode, options.event)

  return {
    data: isExplicitMessage ? data : getErrorData(data),
    msg: shouldExposeMessage(statusCode, options.expose)
      ? message
      : RESPONSE_CODE.label(RESPONSE_CODE.SERVER_ERROR),
