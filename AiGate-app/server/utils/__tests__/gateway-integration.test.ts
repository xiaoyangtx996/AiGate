...
describe('gateway integration', () => {
  ...
  describe('selectChannel', () => {
    it('should return first healthy channel by priority', async () => {
      mockSelect.mockReturnValue(createChannelSelectChain([
        { id: 'ch-1', endpoint: 'https://a.com/v1', vendor: 'openai', priority: 1, status: 'enabled', health: 'down' },
        { id: 'ch-2', endpoint: 'https://b.com/v1', vendor: 'openai', priority: 2, status: 'enabled', health: 'healthy' },
      ]))
  ...
  describe('validateApiKeyFromHeader', () => {
    ...
    it('should return null for inactive or expired keys', async () => {
      mockSelect.mockReturnValueOnce(createKeySelectChain([
        { key: 'ag-key', status: 'revoked', expiresAt: null },
      ]))
      expect(await validateApiKeyFromHeader('Bearer ag-key')).toBeNull()

      mockSelect.mockReturnValueOnce(createKeySelectChain([
        { key: 'ag-exp', status: 'active', expiresAt: new Date('2020-01-01') },
      ]))
  ...
  describe('proxyToChannel', () => {
    it('should call upstream endpoint and return response metadata', async () => {
      ...
      const result = await proxyToChannel(
        { id: 'ch-1', endpoint: 'https://api.example.com/v1/', vendor: 'openai', priority: 1, status: 'enabled', health: 'healthy' },
        'chat/completions',
        'POST',
  ...
  describe('proxyToChannelStream', () => {
    it('should throw when upstream stream fails', async () => {
      ...
      await expect(proxyToChannelStream(
        { id: 'ch-1', endpoint: 'https://api.example.com/v1', vendor: 'openai', priority: 1, status: 'enabled', health: 'healthy' },
        'chat/completions',
        'POST',
        {},
