...
export async function saveMessage(conversationId: string, role: string, content: string, tokens?: number, latency?: number) {
...
export async function sendAgentMessage(agentId: string, userId: string, userMessage: string, conversationId?: string) {
  ...
  await logAgentApiCall(agentId, userId, agentRecord.organizationId, agentRecord.model || 'gpt-4o', channelConfig.vendor, totalTokensFromUsage(usage), latency, result.status)
...
async function logAgentApiCall(agentId: string, userId: string, organizationId: string | null, model: string, provider: string, totalTokens: number, latency: number, statusCode: number) {
...
export async function* streamAgentMessage(agentId: string, userId: string, userMessage: string, conversationId?: string) {
  ...
  await logAgentApiCall(agentId, userId, agentRecord.organizationId, agentRecord.model || 'gpt-4o', channelConfig.vendor, totalTokens, latency, 200)
