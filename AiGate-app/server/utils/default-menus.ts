...
export async function seedDefaultMenus(db: typeof import('@/db/drizzle').db) {
  ...
  if (value === 0) {
    ...
    return { seeded: true, count: defaultMenuSeeds.length, backfilled: 0 }
  }

  const menusWithPermissions = defaultMenuSeeds.filter(seed => seed.id && seed.permissions && seed.permissions > 0)
  ...
  return { seeded: false, count: value, backfilled: menusWithPermissions.length }
}
