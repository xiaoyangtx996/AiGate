...
export function createQuotaRequest(draft: QuotaRequestDraft) {
  if (!draft.organizationId)
    return { valid: false, reason: '缺少组织' }
  if (!draft.requesterId)
    return { valid: false, reason: '缺少申请人' }
...
export function decideQuotaRequest(decision: QuotaApprovalDecision) {
  if (!decision.requestId)
    return { valid: false, reason: '缺少申请记录' }
  if (!decision.approverId)
    return { valid: false, reason: '缺少审批人' }
...
export function validateQuotaConservation(
  nodes: QuotaNode[],
  changes: QuotaChange[] = [],
) {
...
export async function createOrganizationQuotaRequest(draft: QuotaRequestDraft) {
  ...
  const [org] = await db
    .select()
    .from(organization)
    .where(eq(organization.id, draft.organizationId))
  if (!org) {
  ...
  const [created] = await db.insert(quotaRequest).values(
    draft.reason ? { ...values, reason: draft.reason } : values,
  ).returning()
...
export async function decideOrganizationQuotaRequest(decision: QuotaApprovalDecision) {
  ...
  const [updated] = await db.update(quotaRequest).set(
    decision.comment ? { ...updateValues, decisionComment: decision.comment } : updateValues,
  ).where(eq(quotaRequest.id, decision.requestId)).returning()
...
export async function checkQuota(organizationId: string, requestedTokens: number) {
  ...
  if (!org)
    return { allowed: false, reason: '组织不存在' }
  if (org.tokenLimit <= 0)
    return { allowed: true, remaining: Infinity }
  const remaining = org.tokenLimit - org.tokenUsed
  if (remaining <= 0)
    return { allowed: false, reason: '配额已用尽', remaining: 0 }
  if (requestedTokens > remaining)
    return { allowed: false, reason: `配额不足，剩余 ${remaining} tokens`, remaining }
