...
export default defineEventHandler(async (event) => {
  try {
    ...
    const rows = await db.select().from(logs).leftJoin(user, eq(logs.userId, user.id)).where(where).orderBy(desc(logs.createdAt)).limit(5000)
    ...
    const csvRows = rows.map(({ logs: row, user: u }) => [
      ...
      row.os || '',
      row.browser || '',
    ].map(v => `"${String(v).replace(csvQuotePattern, '""')}"`).join(','))
