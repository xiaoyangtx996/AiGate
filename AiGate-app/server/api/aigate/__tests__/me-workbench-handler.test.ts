...
vi.mock('@/db/schema', () => ({
  ...
  apiKey: { id: 'id', name: 'name', status: 'status', env: 'env', expiresAt: 'expiresAt', lastUsed: 'lastUsed', calls: 'calls', userId: 'userId', createdAt: 'createdAt' },
  apiLog: { userId: 'userId', totalTokens: 'totalTokens', cost: 'cost', createdAt: 'createdAt' },
  agent: { id: 'id', name: 'name', description: 'description', model: 'model', status: 'status', builtin: 'builtin', enabled: 'enabled', organizationId: 'organizationId', updatedAt: 'updatedAt' },
  alert: { id: 'id', title: 'title', message: 'message', type: 'type', severity: 'severity', read: 'read', userId: 'userId', organizationId: 'organizationId', createdAt: 'createdAt' },
}))
...
describe('aigate me workbench handler', () => {
  ...
  it('should return current user workbench summary', async () => {
    mockSelect
      ...
      .mockReturnValueOnce(createOrderedLimitChain([{ id: 'alert-1', title: 'Quota', severity: 'warning', read: false }]))
