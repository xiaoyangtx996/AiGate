...
export default defineEventHandler(async (event) => {
  try {
    ...
    const [
      ...
      principal.organizationId
        ? db.select({
          ...
          }).from(organization).where(eq(organization.id, principal.organizationId))
        : Promise.resolve([]),
      ...
      db.select({
        ...
        calls: apiKey.calls,
      }).from(apiKey).where(eq(apiKey.userId, principal.userId)).orderBy(desc(apiKey.createdAt)).limit(5),
      ...
      db.select({
        ...
        .from(agent)
        .where(orgFilter ? and(eq(agent.enabled, true), or(eq(agent.builtin, true), orgFilter)) : eq(agent.enabled, true))
    ...
    const usagePercent = org && org.tokenLimit > 0
      ? Math.round((org.tokenUsed / org.tokenLimit) * 100)
      : 0
