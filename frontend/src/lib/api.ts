...
export type SkillAsset = { id: string; name: string; description: string; active_version_id: string; version: number; instructions: string; hook: object; active: boolean }
export type SkillMemory = { id: string; project_id: string; agent_id: string; skill_id: string; version_id: string; user_id: string; input: string; output: string; trace_id: string }
...
export const api = {
  ...
  skills: () => request<SkillAsset[]>('/v1/skills'),
  createSkill: (payload: object) => request<SkillAsset>('/v1/skills', { method: 'POST', body: JSON.stringify(payload) }),
  createSkillVersion: (skillID: string, payload: object) => request<object>(`/v1/skills/${skillID}/versions`, { method: 'POST', body: JSON.stringify(payload) }),
  grantSkill: (projectID: string, skillID: string) => request<void>(`/v1/projects/${projectID}/skills/${skillID}/grants`, { method: 'PUT' }),
  projectSkills: (projectID: string) => request<SkillAsset[]>(`/v1/projects/${projectID}/skills`),
  skillMemories: (skillID: string) => request<SkillMemory[]>(`/v1/skills/${skillID}/memories`),
  optimizeSkill: (skillID: string) => request<{ status: string }>(`/v1/skills/${skillID}/optimize`, { method: 'POST' }),
