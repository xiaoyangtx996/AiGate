...
function params(limit = 200) {
  const result = new URLSearchParams({ limit: String(limit), ...utcDayRange(filters.from, filters.to) })
...
<template>
  <div class="page page--wide">
    ...
    <section class="section-block section-block--flush">
      <div v-if="logs.length" class="table-wrap"><table class="table--dense"><thead><tr><th>时间 / Trace</th><th>项目</th><th>模型</th><th>Token</th><th>成本</th><th>结果</th><th>状态码</th></tr></thead><tbody>
        <tr v-for="item in logs" :key="item.id"><td><div class="stacked-cell"><strong>{{ new Date(item.created_at).toLocaleString('zh-CN') }}</strong><code :title="item.trace_id">{{ item.trace_id.slice(0, 18) }}{{ item.trace_id.length > 18 ? '…' : '' }}</code></div></td><td>{{ item.project_name || item.project_id || '未归因' }}</td><td>{{ item.model }}</td><td><div class="token-cell"><strong>{{ item.total_tokens.toLocaleString() }}</strong><small>入 {{ item.input_tokens }} / 出 {{ item.output_tokens }}</small></div></td><td><span>{{ money(item.cost_micros) }}</span><small v-if="item.estimated" class="estimate-tag">估算</small></td><td><span class="status" :class="item.blocked ? 'status--danger' : item.status_code >= 400 ? 'status--warning' : 'status--success'">{{ item.blocked ? '配额拦截' : item.status_code >= 400 ? '上游失败' : '成功' }}</span><small v-if="item.error_code" class="error-code">{{ item.error_code }}</small></td><td><code>{{ item.status_code }}</code></td></tr>
      </tbody></table></div>
