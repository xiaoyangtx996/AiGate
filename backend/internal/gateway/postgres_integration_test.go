...
func TestPostgresLoggerWritesCorrelatedBlockedAudit(t *testing.T) {
	...
	if err = NewPostgresLogger(store).Write(ctx, Log{TraceID: trace, TenantID: tenant, OrganizationID: organization, UserID: user, APIKeyID: key, Model: "test-model", Estimated: true, Blocked: true, StatusCode: 429, ErrorCode: "quota_exhausted"}); err != nil {
		t.Fatal(err)
	}
