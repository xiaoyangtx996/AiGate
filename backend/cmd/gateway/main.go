...
func main() {
	...
	h := &gateway.Handler{Keys: apikey.NewService(apikey.NewPostgres(store)), Quota: quota.NewService(quota.NewPostgres(store), alertService), Channels: channel.NewService(channel.NewPostgres(store), cipher), Logs: gateway.NewPostgresLogger(store), Projects: store, Client: &http.Client{Timeout: 2 * time.Minute}, TrustedProxyCIDRs: split(os.Getenv("TRUSTED_PROXY_CIDRS"))}
