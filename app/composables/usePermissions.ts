...
export function usePermissions() {
  ...
  function getPermissionValues(bits: number) {
    if (!bits) return []
    return map(
      PERMISSIONS.items.filter(({ raw }) => (bits & raw.bits) === raw.bits),
      'value',
    )
  }
  ...
  function getPermissionBits(values: string[]) {
    return PERMISSIONS.items.filter(({ value }) => values.includes(value)).reduce((sum, { raw }) => sum | raw.bits, 0)
  }
  ...
  function buildRolePermissions(checkedKeys: string[]) {
    ...
    const permissionBitMap = new Map(PERMISSIONS.items.map(p => [p.value, p.raw.bits]))
    ...
    return menus.flatMap(menu => {
      ...
      const permissionKeys =
        menu.permissions && menu.permissions !== 0
          ? getPermissionValues(menu.permissions).map(value => ({ id: `${menu.id}:${value}` }))
          : []

      // 子菜单
      const childrenKeys = menu.children?.length ? getAllPermissionKeys(menu.children) : []
