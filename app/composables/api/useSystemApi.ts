...
export function useSystemApi() {
   ...
   * @description: 查询菜单
   */
  const getMenuList = (params?: MenuQueryParams) => get<MenuTree[]>('/system-settings/menu-manage', params)

  /**
   * @description: 新增菜单
   */
  const insertMenu = (body: InsertMenu) => post<Menu>('/system-settings/menu-manage', body)
   ...
   * @description: 删除菜单
   */
  const delMenu = (id: string) => del<Menu>(`/system-settings/menu-manage/${id}`)
   ...
   * @description: 新增角色
   */
  const insertRole = (body: InsertRole) => post<Role>('/system-settings/role-manage', body)
   ...
   * @description: 删除角色
   */
  const delRole = (id: string) => del<Role>(`/system-settings/role-manage/${id}`)
   ...
   * @description: 批量删除
   */
  const delLogs = (params: { ids: string[] }) => del<Log[]>('/system-settings/operation-log', {}, { body: params })
   ...
   */
  const getLocales = () => get<Record<Locale, any>>('/system-settings/internalization/locales')
   ...
   * @description: 删除国际化
   */
  const delInternalization = (id: string) => del<Internalization>(`/system-settings/internalization/${id}`)

  /**
   * @description: 获取角色权限
   */
  const getRolePermissions = (roleId: string) => get<RoleMenu[]>(`/system-settings/role-manage/${roleId}/permissions`)
