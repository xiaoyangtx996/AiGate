        ...
        return val
          ? h(
              ...
              val
                .split(',')
                .map((v: string) =>
                  h(UBadge, { variant: 'soft', color: 'info' }, () => i18nUser(`role${upperFirst(v)}`)),
                ),
            )
          : '-'
      },
        ...
        return h(
          UDropdownMenu,
          {
            ...
            items: [
              ...
              {
                label: i18nPermissions(
                  banned ? PERMISSIONS.label(PERMISSIONS.UNBAN_USER) : PERMISSIONS.label(PERMISSIONS.BAN_USER),
                ),
                icon: banned
                  ? PERMISSIONS.raw(PERMISSIONS.UNBAN_USER).icon
                  : PERMISSIONS.raw(PERMISSIONS.BAN_USER).icon,
