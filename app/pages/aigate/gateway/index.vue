...
const {
  ...
  pending: loading,
  refresh,
} = await useAsyncData('gateway-overview', async () => {
  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <UCard>
        ...
        <p class="text-2xl font-bold">{{ overview.activeKeys }}/{{ overview.totalKeys }}</p>
      </UCard>
      <UCard>
        <p class="text-sm text-muted">
          {{ p('healthyChannels') }}
        </p>
        <p class="text-2xl font-bold text-success">{{ overview.healthyChannels }}/{{ overview.activeChannels }}</p>
      </UCard>
      <UCard>
        <p class="text-sm text-muted">
          {{ p('requestsLastHour') }}
      ...
      <UCard>
        ...
        <p class="text-2xl font-bold">{{ overview.avgLatency || 0 }}ms</p>
      </UCard>
    </div>
