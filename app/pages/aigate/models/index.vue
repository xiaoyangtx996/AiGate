      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <UCard v-for="model in list" :key="model.id" class="hover:border-primary transition-colors">
          ...
          <div class="space-y-2 text-sm">
            ...
            <div class="flex justify-between">
              <span class="text-muted">{{ p('context') }}</span
              ><span class="font-mono">{{ model.contextWindow?.toLocaleString() }} tokens</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ p('inputPrice') }}</span
              ><span class="font-mono">${{ model.inputPrice }}/1K</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ p('outputPrice') }}</span
              ><span class="font-mono">${{ model.outputPrice }}/1K</span>
            </div>
          </div>
