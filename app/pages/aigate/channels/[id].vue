    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <UCard>
          ...
          <div class="space-y-3 text-sm">
            <div class="flex justify-between">
              <span class="text-muted">{{ $t('pages.aigate.channels.name') }}</span
              ><span>{{ channel?.name }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ $t('pages.aigate.channels.vendor') }}</span
              ><span>{{ channel?.vendor }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ $t('pages.aigate.channels.endpoint') }}</span
              ><code class="text-xs">{{ channel?.endpoint }}</code>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ $t('pages.aigate.channels.apiKey') }}</span
              ><code class="text-xs">{{ maskKey(channel?.apiKey) }}</code>
            </div>
            <div class="flex justify-between">
            ...
            <div class="flex justify-between">
              <span class="text-muted">{{ p('priority') }}</span
              ><span>{{ channel?.priority }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ p('weight') }}</span
        ...
        <UCard>
          ...
          <div class="space-y-3 text-sm">
            <div class="flex justify-between">
              <span class="text-muted">{{ $t('pages.aigate.channels.qps') }}</span
              ><span>{{ channel?.qps }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ p('rateLimitQps') }}</span
              ><span>{{ channel?.rateLimitQps }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ p('rateLimitTpm') }}</span
              ><span>{{ channel?.rateLimitTpm }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ p('rateLimitRpm') }}</span
              ><span>{{ channel?.rateLimitRpm }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">{{ p('strategy') }}</span
              ><span>{{ channel?.rateLimitStrategy }}</span>
            </div>
          </div>
        </UCard>
      </div>
