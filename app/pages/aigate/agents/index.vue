...
const { selectedCount, hasSelection, isSelected, toggleSelect, batchDelete } = useBatchOperations<AgentRow>({
      ...
      <UCard v-for="agent in list" :key="agent.id" :class="agent.builtin ? 'border-primary' : ''">
        <div class="relative">
          <div v-if="!agent.builtin" class="absolute top-0 left-0 z-10">
            <UCheckbox :model-value="isSelected(agent.id)" @update:model-value="toggleSelect(agent.id)" />
          </div>
