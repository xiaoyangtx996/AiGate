...
const {
  data: requestData,
  pending: requestLoading,
  refresh: refreshRequests,
} = await useAsyncData(
  'aigate-my-quota-requests',
  async () => {
...
async function submitQuotaRequest() {
  if (!quotaForm.requestedTokenLimit) return
    ...
    <template v-else>
      ...
      <UCard>
        ...
        <div v-if="quota" class="space-y-3">
          ...
          <p class="text-sm text-muted">{{ p('remaining') }}：{{ formatTokens(quota.remaining) }}</p>
        </div>
      ...
      <UCard>
        ...
        <div v-else-if="quotaRequests.length" class="space-y-2">
          <div
            v-for="request in quotaRequests"
            :key="request.id"
