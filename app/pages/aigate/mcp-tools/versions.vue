      ...
      <template v-if="selectedTool">
        <UCard class="mb-4">
          ...
          <div class="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span class="text-muted">{{ p('type') }}：</span>{{ selectedTool.type }}
            </div>
            <div>
              <span class="text-muted">{{ p('calls') }}：</span>{{ selectedTool.usageCount || 0 }}
            </div>
            <div>
              <span class="text-muted">{{ p('endpoint') }}：</span
              ><code class="text-xs">{{ selectedTool.config?.endpoint || '-' }}</code>
            </div>
            <div>
              <span class="text-muted">{{ p('lastCheck') }}：</span
              >{{ selectedTool.lastHealthCheck ? new Date(selectedTool.lastHealthCheck).toLocaleString() : '-' }}
            </div>
          </div>
        ...
        <UCard>
          ...
          <div v-if="versions.length > 0" class="space-y-3">
            <div v-for="ver in versions" :key="ver.id" class="flex items-center gap-3 p-3 rounded-lg border">
              <UIcon
                :name="ver.active ? 'lucide:check-circle' : 'lucide:git-commit'"
                :class="ver.active ? 'text-success' : 'text-muted'"
              />
              <div class="flex-1">
                <div class="flex items-center gap-2">
