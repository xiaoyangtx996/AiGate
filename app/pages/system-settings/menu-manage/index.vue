...
const {
  ...
  pending: loading,
  refresh,
} = await useAsyncData('menu-manage', async () => {
...
async function handleSubmit(values: InsertMenu) {
  ...
  await (editData.value?.id ? updateMenu({ ...values, id: editData.value.id }) : insertMenu(values))
    .then(({ code }) => {
      if (isSuccess(code)) {
...
<template>
  <div class="space-y-4">
    <ClientOnly>
      <HeaderContent v-model="keyword" :refresh :handle-add :loading :table="table?.tableApi" />
    </ClientOnly>
    <UTable
      ref="table"
