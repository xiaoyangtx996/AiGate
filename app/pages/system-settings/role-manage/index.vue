...
const {
  ...
  pending: loading,
  refresh,
} = await useAsyncData('role-manage', async () => {
...
async function handleSubmit(values: InsertRole) {
  ...
  await (editData.value?.id ? updateRole({ ...values, id: editData.value.id }) : insertRole(values))
    .then(({ code }) => {
      if (isSuccess(code)) {
...
<template>
  <div class="space-y-4">
    <ClientOnly>
      <HeaderContent v-model="query" :refresh :handle-add :loading :table="table?.tableApi" />
    </ClientOnly>
    <UTable
      ref="table"
      ...
      <template #expanded="{ row }">
        <div v-if="row.original.menus.length" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div v-for="item in row.original.menus" :key="item.menuId" class="flex items-center gap-2 flex-wrap">
            <span class="font-medium min-w-24"> {{ $t(item.menu.label) }}: </span>
