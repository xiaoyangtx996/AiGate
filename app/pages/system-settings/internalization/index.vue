...
const {
  ...
  pending: loading,
  refresh,
} = await useAsyncData('internalization', async () => {
...
async function handleSubmit(values: InternalizationFormSchema) {
  ...
  await (
    editData.value?.id ? updateInternalization({ ...values, id: editData.value.id }) : insertInternalization(values)
  )
    .then(({ code }) => {
      if (isSuccess(code)) {
...
<template>
  <div class="space-y-4">
    <ClientOnly>
      <HeaderContent v-model="query" :refresh :handle-reset :handle-add :loading :table="table?.tableApi" />
    </ClientOnly>
    <UTable
      ref="table"
