...
const classes = computed(() => {
  return ['spinner', props.size && `spinner--${props.size}`, `text-${props.color || 'primary'}`, props.class]
})
</script>
...
<template>
  <span :class="classes" data-slot="spinner">
    <svg data-slot="spinner-icon" viewBox="0 0 24 24" aria-hidden="true" role="presentation">
      <defs>
        <linearGradient :id="`gradient-1-${uid}`" x1="50%" x2="50%" y1="5.271%" y2="91.793%">
          <stop offset="0%" stop-color="currentColor" />
          <stop offset="100%" stop-color="currentColor" stop-opacity="0.55" />
        </linearGradient>

        <linearGradient :id="`gradient-2-${uid}`" x1="50%" x2="50%" y1="15.24%" y2="87.15%">
