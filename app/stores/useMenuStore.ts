...
export const useMenuStore = defineStore('menu-store', () => {
  ...
  const init = async () => {
    if (inited.value && menuTree.value.length > 0) return
    await fetchMenuTree()
  }
  ...
  const normalizeRouteName = (to: string) => {
    if (!to || to === '/') return 'index'

    return to.replace(LEADING_SLASH_RE, '').replace(TRAILING_SLASH_RE, '').split('/').filter(Boolean).join('-')
  }
