...
| **7 系统设置聚合与全局体验**   | system_setting 配置中心（7 Tab）+ 全局搜索 ⌘K + 个人功能 Drawer（自助改密码/通知偏好）+ 用户批量导入与离职向导 + 预警 12 类  | `2026-06-10-settings-global-ux-design.md`     |
| **8 AiGate Bot（管理 Agent）** | 内置只读统计工具集 + 顶栏 Drawer 对话面板 + 权限过滤可视化（复用阶段 4 对话基础设施）                                        | `2026-06-10-aigate-bot-design.md`             |

依赖关系：阶段 0+1 是所有后续阶段的前提（错误链路与租户上下文）；阶段 2 的「渠道同步模型」解锁模型页数据；阶段 3 与阶段 2 无强依赖可并行；阶段 4 依赖阶段 2（embedding/rerank 模型经网关调用）；阶段 5 可与 3/4 并行；阶段 6/7 各 WP 互不依赖、可穿插排期（个别 Tab/功能依赖对应阶段就绪）；阶段 8 依赖阶段 4 的对话基础设施。各阶段实施粒度：阶段 4 体量大，实施时拆 4A（知识库）/ 4B（Skills+Agent）两个计划。
