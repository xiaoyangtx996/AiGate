...
export function normalizeNotifyChannels(channels: unknown): AlertNotifyChannel[] {
  if (!Array.isArray(channels)) return [...defaultTemplate.notifyChannels]
...
export function normalizeAlertRuleUpdateInput(body: {
  ...
  const template = getAlertRuleTemplate(body.condition?.templateId)
  if ('type' in body || body.condition?.templateId) update.type = body.type || template.type
  ...
  if ('notifyChannels' in body) update.notifyChannels = normalizeNotifyChannels(body.notifyChannels)
